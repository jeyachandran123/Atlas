...
class ConversationGateway:
    ...
    async def ask(
        ...
        try:
            prepared = await self._prepare(ctx, collector)
    ...
    async def _prepare(
        ...
        """Intent → plan → retrieve → rank → context → prompt. Returns either
        a (prompt, bundle) pair, or a TurnResult refusal that ends the turn
        ...
        if not retrieval.chunks:
            return self._refusal(ctx, "no_knowledge_found", REFUSAL_SENTENCE)
        ...
        if bundle.best_confidence < self._min_score:
            return self._refusal(ctx, "low_confidence", REFUSAL_SENTENCE)
